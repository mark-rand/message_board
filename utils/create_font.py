...
while running:
    ...
    fonts = {"Px437 Sigma RM 8x8": {'height': 8, 'start': 8, 'end': 0},
             "Px437 ToshibaSat 8x8": {'height': 8, 'start': 8, 'end': 0},
             "Px437_HP_100LX_10x11": {'height': 11, 'start': 10, 'end': 1},
             "CG Pixel 4x5": {'height': 5, 'start': 5, 'end': 0}}
    all_fonts = {}
    WHITE = (255, 255, 255)
    BLACK = (0, 0, 0)
    for font in fonts:
        ...
        for character in range(33, 173):
            ...
            # Trim down the font
            non_blank_indexes = [index for index in range(
                0, len(thisChar)) if any(x.isalnum() for x in thisChar[index])]
            if len(non_blank_indexes) > 0:
                definition = thisChar[min(non_blank_indexes):
                                      max(non_blank_indexes)+1]
                fontchars[chr(character)] = definition
            else:
                thisChar
